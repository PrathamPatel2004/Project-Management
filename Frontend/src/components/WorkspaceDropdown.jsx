...
import { fetchProjects, clearProjects } from "../features/projectSlice";
...
function WorkspaceDropdown() {
    const { workspaces = [], currentWorkspace = null, loading } = useSelector((state) => state.workspace || {});
            ...
            <div className="relative m-4" ref={dropdownRef}>
                    ...
                    >
                        <div className="flex items-center gap-3">
                            ...
                            <div className="text-left min-w-0">
                                <p className="font-semibold text-sm truncate">{currentWorkspace?.name || "Select Workspace"}</p>
                                <p className="text-xs text-gray-500 dark:text-neutral-400">{workspaces.length} workspace {workspaces.length !== 1 ? "s" : ""}</p>
                            </div>
                        </div>
                ...
                {isOpen && (
                    <div className="absolute z-50 w-64 bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-700 rounded shadow top-full left-0">
                        <div className="p-2">
                            <p className="text-xs text-gray-500 dark:text-neutral-400 uppercase tracking-wide mb-2 px-2">Workspaces</p>
                            {loading && (
                                <p className="text-xs px-2 py-1 text-gray-400">Loading...</p>
                            )}
                            {!loading && workspaces.length === 0 ? (
                                <div className="p-2">No Workspaces</div>
                            ) : (
                                workspaces.map((ws) => (
                                    <div
                                        key={ws.id}
                                        onClick={() => onSelectWorkspace(ws.id)}
                                        className="flex items-center gap-3 p-2 cursor-pointer rounded hover:bg-gray-100 dark:hover:bg-neutral-800"
                                    >
                                        <img
                                            src={ws.logo || "/no-image.png"}
